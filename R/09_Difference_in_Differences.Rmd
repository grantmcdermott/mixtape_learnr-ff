...
library(haven)				## Stata data types and classes
library(data.table)   ## High-performance data wrangling
library(fixest)       ## High-performance (fixed-effect +) regressions
...
drop_cols = grep("^r20|^trend|^lead|^lag|^_Iyear", names(castle), value = TRUE)
...
## Regression macros
setFixest_fml(
	..xvar = ~ blackm_15_24 + whitem_15_24 + blackm_25_44 + whitem_25_44 +   ## demographics
...
dd_cas = feols(l_homicide ~ post + ..xvar | sid[year] + region^year, 
							 data = castle, vcov = ~sid, weights = ~popwt)
summary(dd_cas)
```
...
## date.
castle[, ':=' (time_til = year - treatment_date, treated = max(post)), by = sid
...
**GM: `iplot()` is _extremely_ customizable. But you might also want to check out [`ggiplot`](https://github.com/grantmcdermott/ggiplot) for a ggplot2 equivalent.**
...
bacon_cas = bacon(l_homicide ~ post,
									...
									data = castle[, .(l_homicide, post, state, year)])
